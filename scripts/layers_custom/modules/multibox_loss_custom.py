...
from ..box_utils_custom import match_custom, log_sum_exp, decode, center_size, crop, elemwise_mask_iou, elemwise_box_iou

from data_custom import cfg_custom, mask_type_custom, activation_func

class MultiBoxLoss_custom(nn.Module):
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        super(MultiBoxLoss_custom, self).__init__()
        ...
        if cfg_custom.use_class_balanced_conf:
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        Args:
            ...
            * Only if mask_type_custom == lincomb
        """
        ...
        if cfg_custom.mask_type_custom == mask_type_custom.lincomb:
            proto_data = predictions['proto']

        score_data = predictions['score'] if cfg_custom.use_mask_scoring   else None   
        inst_data  = predictions['inst']  if cfg_custom.use_instance_coeff else None
        ...
        if cfg_custom.train_boxes:
            loc_p = loc_data[pos_idx].view(-1, 4)
            loc_t = loc_t[pos_idx].view(-1, 4)
            losses['B'] = F.smooth_l1_loss(loc_p, loc_t, reduction='sum') * cfg_custom.bbox_alpha

        if cfg_custom.train_masks:
            if cfg_custom.mask_type_custom == mask_type_custom.direct:
                if cfg_custom.use_gt_bboxes:
                    pos_masks = []
                    for idx in range(batch_size):
                        pos_masks.append(masks[idx][idx_t[idx, pos[idx]]])
                    masks_t = torch.cat(pos_masks, 0)
                    masks_p = mask_data[pos, :].view(-1, cfg_custom.mask_dim)
                    losses['M'] = F.binary_cross_entropy(torch.clamp(masks_p, 0, 1), masks_t, reduction='sum') * cfg_custom.mask_alpha
                else:
                ...
                if cfg_custom.mask_proto_loss is not None:
                    if cfg_custom.mask_proto_loss == 'l1':
                        losses['P'] = torch.mean(torch.abs(proto_data)) / self.l1_expected_area * self.l1_alpha
                    elif cfg_custom.mask_proto_loss == 'disj':
        ...
        # Mask IoU Loss
        if cfg_custom.use_maskiou and maskiou_targets is not None:
    ...
    def class_existence_loss(self, class_data, class_existence_t):
        return cfg_custom.class_existence_alpha * F.binary_cross_entropy_with_logits(class_data, class_existence_t, reduction='sum')

    def semantic_segmentation_loss(self, segment_data, mask_t, class_t, interpolation_mode='bilinear'):
        # Note num_classes here is without the background class so cfg_custom.num_classes-1
        ...
        return loss_s / mask_h / mask_w * cfg_custom.semantic_segmentation_alpha


    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        # Compute max conf across batch for hard negative mining
        batch_conf = conf_data.view(-1, self.num_classes)
        if cfg_custom.ohem_use_most_confident:
        ...
        return cfg_custom.conf_alpha * loss_c
    ...
    def focal_conf_loss(self, conf_data, conf_t):
        ...
        at = (1 - cfg_custom.focal_loss_alpha) * background + cfg_custom.focal_loss_alpha * (1 - background)

        loss = -at * (1 - pt) ** cfg_custom.focal_loss_gamma * logpt

        # See comment above for keep
        return cfg_custom.conf_alpha * (loss * keep).sum()
    ...
    def focal_conf_sigmoid_loss(self, conf_data, conf_t):
        ...
        at = cfg_custom.focal_loss_alpha * conf_one_t + (1 - cfg_custom.focal_loss_alpha) * (1 - conf_one_t)
        at[..., 0] = 0 # Set alpha for the background class to 0 because sigmoid focal loss doesn't use it

        loss = -at * (1 - pt) ** cfg_custom.focal_loss_gamma * logpt
        loss = keep * loss.sum(dim=-1)

        return cfg_custom.conf_alpha * loss.sum()
    ...
    def focal_conf_objectness_loss(self, conf_data, conf_t):
        ...
        obj_loss = -at * (1 - pt) ** cfg_custom.focal_loss_gamma * logpt
        ...
        return cfg_custom.conf_alpha * (class_loss + (obj_loss * keep).sum())
    ...
    def conf_objectness_loss(self, conf_data, conf_t, batch_size, loc_p, loc_t, priors):
        ...
        with torch.no_grad():
            ...
            boxes_pred = decode(loc_p, pos_priors, cfg_custom.use_yolo_regressors)
            boxes_targ = decode(loc_t, pos_priors, cfg_custom.use_yolo_regressors)
        ...
        return cfg_custom.conf_alpha * (class_loss + obj_pos_loss + obj_neg_loss)
    ...
    def direct_mask_loss(self, pos_idx, idx_t, loc_data, mask_data, priors, masks):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                pos_bboxes = decode(loc_data[idx, :, :], priors.data, cfg_custom.use_yolo_regressors)
                ...
                for jdx in range(num_pos):
                    ...
                    new_mask = F.adaptive_avg_pool2d(tmp_mask.unsqueeze(0), cfg_custom.mask_size)
            ...
            loss_m += F.binary_cross_entropy(torch.clamp(pos_mask_data, 0, 1), mask_t, reduction='sum') * cfg_custom.mask_alpha
    ...
    def coeff_diversity_loss(self, coeffs, instance_t):
        ...
        return cfg_custom.mask_proto_coeff_diversity_alpha * loss.sum() / num_pos
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, interpolation_mode='bilinear'):
        ...
        process_gt_bboxes = cfg_custom.mask_proto_normalize_emulate_roi_pooling or cfg_custom.mask_proto_crop

        if cfg_custom.mask_proto_remove_empty_masks:
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                if cfg_custom.mask_proto_binarize_downsampled_gt:
                    downsampled_masks = downsampled_masks.gt(0.5).float()
                ...
                if cfg_custom.mask_proto_reweight_mask_loss:
                    # Ensure that the gt is binary
                    if not cfg_custom.mask_proto_binarize_downsampled_gt:
                    ...
                    mask_reweighting   = gt_foreground_norm * cfg_custom.mask_proto_reweight_coeff + gt_background_norm
            ...
            if process_gt_bboxes:
                ...
                if cfg_custom.mask_proto_crop_with_pred_box:
                    pos_gt_box_t = decode(loc_data[idx, :, :], priors.data, cfg_custom.use_yolo_regressors)[cur_pos]
                else:
            ...
            if cfg_custom.mask_proto_coeff_diversity_loss:
            ...
            if old_num_pos > cfg_custom.masks_to_train:
                perm = torch.randperm(proto_coef.size(0))
                select = perm[:cfg_custom.masks_to_train]
            ...
            pred_masks = cfg_custom.mask_proto_mask_activation(pred_masks)

            if cfg_custom.mask_proto_double_loss:
                if cfg_custom.mask_proto_mask_activation == activation_func.sigmoid:
                ...
                loss_m += cfg_custom.mask_proto_double_loss_alpha * pre_loss

            if cfg_custom.mask_proto_crop:
            ...
            if cfg_custom.mask_proto_normalize_mask_loss_by_sqrt_area:
            ...
            if cfg_custom.mask_proto_normalize_emulate_roi_pooling:
                weight = mask_h * mask_w if cfg_custom.mask_proto_crop else 1
            ...
            if cfg_custom.use_maskiou:
                if cfg_custom.discard_mask_area > 0:
                    gt_mask_area = torch.sum(mask_t, dim=(0, 1))
                    select = gt_mask_area > cfg_custom.discard_mask_area
        ...
        losses = {'M': loss_m * cfg_custom.mask_alpha / mask_h / mask_w}
        ...
        if cfg_custom.use_maskiou:
            ...
            if cfg_custom.maskious_to_train > 0 and num_samples > cfg_custom.maskious_to_train:
    ...
    def mask_iou_loss(self, net, maskiou_targets):
        ...
        return loss_i * cfg_custom.maskiou_alpha
