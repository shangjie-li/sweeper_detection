...
from data_custom import cfg_custom
...
def match_custom(pos_thresh, neg_thresh, truths, priors, labels, crowd_boxes, loc_t, conf_t, idx_t, idx, loc_data):
    ...
    """
    decoded_priors = decode(loc_data, priors, cfg_custom.use_yolo_regressors) if cfg_custom.use_prediction_matching else point_form(priors)
    
    # Size [num_objects, num_priors]
    overlaps = jaccard(truths, decoded_priors) if not cfg_custom.use_change_matching else change(truths, decoded_priors)
    ...
    if crowd_boxes is not None and cfg_custom.crowd_iou_threshold < 1:
        # Size [num_priors, num_crowds]
        crowd_overlaps = jaccard(decoded_priors, crowd_boxes, iscrowd=True)
        # Size [num_priors]
        best_crowd_overlap, best_crowd_idx = crowd_overlaps.max(1)
        # Set non-positives with crowd iou of over the threshold to be neutral.
        conf[(conf <= 0) & (best_crowd_overlap > cfg_custom.crowd_iou_threshold)] = -1

    loc = encode(matches, priors, cfg_custom.use_yolo_regressors)
