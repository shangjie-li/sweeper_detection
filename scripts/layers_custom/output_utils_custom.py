...
from data_custom import cfg_custom, mask_type_custom, MEANS, STD, activation_func
from utils_custom.augmentations_custom import Resize_custom
from utils_custom import timer
from .box_utils_custom import crop, sanitize_coordinates
...
def postprocess_custom(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if cfg_custom.mask_type_custom == mask_type_custom.lincomb and cfg_custom.eval_mask_branch:
        ...
        if cfg_custom.mask_proto_debug:
            np.save('scripts/proto.npy', proto_data.cpu().numpy())
        
        if visualize_lincomb:
            display_lincomb_custom(proto_data, masks)

        masks = proto_data @ masks.t()
        masks = cfg_custom.mask_proto_mask_activation(masks)
        ...
        if cfg_custom.use_maskiou:
            with timer.env('maskiou_net'):                
                with torch.no_grad():
                    ...
                    if cfg_custom.rescore_mask:
                        if cfg_custom.rescore_bbox:
    ...
    if cfg_custom.mask_type_custom == mask_type_custom.direct and cfg_custom.eval_mask_branch:
        # Upscale masks
        ...
        for jdx in range(masks.size(0)):
            ...
            mask = masks[jdx, :].view(1, 1, cfg_custom.mask_size, cfg_custom.mask_size)
...
def undo_image_transformation_custom(img, w, h):
    """
    ...
    if cfg_custom.backbone.transform.normalize:
        img_numpy = (img_numpy * np.array(STD) + np.array(MEANS)) / 255.0
    elif cfg_custom.backbone.transform.subtract_means:
...
def display_lincomb_custom(proto_data, masks):
    out_masks = torch.matmul(proto_data, masks.t())
    # out_masks = cfg_custom.mask_proto_mask_activation(out_masks)
