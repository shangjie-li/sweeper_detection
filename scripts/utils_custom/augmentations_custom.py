...
from data_custom import cfg_custom, MEANS, STD
...
class Resize_custom(object):
    ...
    def __init__(self, resize_gt=True):
        ...
        self.max_size = cfg_custom.max_size
        self.preserve_aspect_ratio = cfg_custom.preserve_aspect_ratio

    def __call__(self, image, masks, boxes, labels=None):
        img_h, img_w, _ = image.shape
        
        if self.preserve_aspect_ratio:
            width, height = Resize_custom.calc_size_preserve_ar(img_w, img_h, self.max_size)
        else:
        ...
        keep = (w > cfg_custom.discard_box_width) * (h > cfg_custom.discard_box_height)
        masks = masks[keep]
        boxes = boxes[keep]
...
class PrepareMasks(object):
    ...
    function doesn't do anything if cfg_custom.use_gt_bboxes is False.
    """
...
class BaseTransform_custom(object):
    """ Transorm to be used when evaluating. """

    def __init__(self, mean=MEANS, std=STD):
        self.augment = Compose([
            ConvertFromInts(),
            Resize_custom(resize_gt=False),
            BackboneTransform(cfg_custom.backbone.transform, mean, std, 'BGR')
        ])
...
class FastBaseTransform_custom(torch.nn.Module):
    """
    ...
    def __init__(self):
        ...
        self.transform = cfg_custom.backbone.transform
    ...
    def forward(self, img):
        ...
        if cfg_custom.preserve_aspect_ratio:
            _, h, w, _ = img.size()
            img_size = Resize_custom.calc_size_preserve_ar(w, h, cfg_custom.max_size)
            img_size = (img_size[1], img_size[0]) # Pytorch needs h, w
        else:
            img_size = (cfg_custom.max_size, cfg_custom.max_size)
...
class SSDAugmentation_custom(object):
    ...
    def __init__(self, mean=MEANS, std=STD):
        self.augment = Compose([
            ConvertFromInts(),
            ToAbsoluteCoords(),
            enable_if(cfg_custom.augment_photometric_distort, PhotometricDistort()),
            enable_if(cfg_custom.augment_expand, Expand(mean)),
            enable_if(cfg_custom.augment_random_sample_crop, RandomSampleCrop()),
            enable_if(cfg_custom.augment_random_mirror, RandomMirror()),
            enable_if(cfg_custom.augment_random_flip, RandomFlip()),
            enable_if(cfg_custom.augment_random_flip, RandomRot90()),
            Resize_custom(),
            enable_if(not cfg_custom.preserve_aspect_ratio, Pad(cfg_custom.max_size, cfg_custom.max_size, mean)),
            ToPercentCoords(),
            PrepareMasks(cfg_custom.mask_size, cfg_custom.use_gt_bboxes),
            BackboneTransform(cfg_custom.backbone.transform, mean, std, 'BGR')
        ])
