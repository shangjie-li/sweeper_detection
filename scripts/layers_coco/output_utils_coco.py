...
from data_coco import cfg_coco, mask_type_coco, MEANS, STD, activation_func
from utils_coco.augmentations_coco import Resize_coco
from utils_coco import timer
from .box_utils_coco import crop, sanitize_coordinates

def postprocess_coco(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if cfg_coco.mask_type_coco == mask_type_coco.lincomb and cfg_coco.eval_mask_branch:
        ...
        if cfg_coco.mask_proto_debug:
            np.save('scripts/proto.npy', proto_data.cpu().numpy())
        
        if visualize_lincomb:
            display_lincomb_coco(proto_data, masks)

        masks = proto_data @ masks.t()
        masks = cfg_coco.mask_proto_mask_activation(masks)
        ...
        if cfg_coco.use_maskiou:
            with timer.env('maskiou_net'):                
                with torch.no_grad():
                    ...
                    if cfg_coco.rescore_mask:
                        if cfg_coco.rescore_bbox:
    ...
    if cfg_coco.mask_type_coco == mask_type_coco.direct and cfg_coco.eval_mask_branch:
        # Upscale masks
        ...
        for jdx in range(masks.size(0)):
            ...
            mask = masks[jdx, :].view(1, 1, cfg_coco.mask_size, cfg_coco.mask_size)
...
def undo_image_transformation_coco(img, w, h):
    """
    ...
    if cfg_coco.backbone.transform.normalize:
        img_numpy = (img_numpy * np.array(STD) + np.array(MEANS)) / 255.0
    elif cfg_coco.backbone.transform.subtract_means:
...
def display_lincomb_coco(proto_data, masks):
    out_masks = torch.matmul(proto_data, masks.t())
    # out_masks = cfg_coco.mask_proto_mask_activation(out_masks)
